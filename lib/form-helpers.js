...
/**
 * UTC日付を日本時間（JST）に変換する
 * @param {Date|string} utcDate - UTC日付オブジェクトまたは日付文字列
 * @returns {Date} 日本時間に変換されたDateオブジェクト
 */
export function convertToJST(utcDate) {
...
/**
 * 現在の日本時間を取得する
 * @returns {Date} 現在の日本時間のDateオブジェクト
 */
export function getCurrentJST() {
    const utcNow = new Date();
    return convertToJST(utcNow);
}