...
export default function EditBroadcast() {
            ...
            try {
                ...
                const { data: broadcast, error: broadcastError } = await supabase
                    ...
                    .select(`
                        ...
                        work:work_id (id, title, category_id),
                        station:station_id (id, name),
                        weekday:weekday_id (id, name)
                    `)
                    .eq('id', id)
                    .single();
                ...
                const formattedData = {
                    id: broadcast.id,
                    workId: broadcast.work_id,
                    stationId: broadcast.station_id,
                    weekdayId: broadcast.weekday_id,
                    broadcastStartDate: broadcast.broadcast_start_date ? broadcast.broadcast_start_date.substring(0, 10) : '',
                    broadcastEndDate: broadcast.broadcast_end_date ? broadcast.broadcast_end_date.substring(0, 10) : '',
                    displayBroadcastTime: broadcast.display_broadcast_time || ''
                };
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            ...
            console.error('放送情報更新エラー:', error);
            alert('放送情報の更新に失敗しました: ' + error.message);
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        <AdminLayout>
            <Head>
                <title>放送情報編集 | 佐藤拓也ファンサイト 管理画面</title>
            </Head>
            <div className="py-4">
                <div className="mb-6">
                    <h1 className="text-2xl font-bold text-primary">放送情報編集</h1>
                    <p className="text-gray-600">テレビ、ラジオ、WEB配信などの放送情報を編集します</p>
                </div>
